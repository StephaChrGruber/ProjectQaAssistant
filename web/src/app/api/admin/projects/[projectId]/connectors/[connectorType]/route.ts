...
export async function PUT(
    req: Request,
    ctx: { params: Promise<{ projectId: string; connectorType: string }> }
) {
    const { projectId, connectorType } = await ctx.params
    const body = await req.text()
    const type = connectorType === "git" ? "github" : connectorType

    const res = await fetch(
        `${BACKEND}/admin/projects/${encodeURIComponent(projectId)}/connectors/${encodeURIComponent(type)}`,
        {
            method: "PUT",
