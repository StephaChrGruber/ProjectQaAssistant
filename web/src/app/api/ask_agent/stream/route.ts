import { backendUrl, devProxyHeaders } from "@/lib/http/backend-proxy"
...
export async function POST(req: Request) {
  ...
  const payload = {
    ...
    workspace_context: body.workspace_context ?? body.workspaceContext ?? null,
  }

  const upstream = await fetch(backendUrl("/ask_agent/stream"), {
    method: "POST",
    headers: devProxyHeaders({ "Content-Type": "application/json" }),
