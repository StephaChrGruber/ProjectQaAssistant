...
const DOC_CONTEXT_MAX_FILES = 50
const DOC_CONTEXT_MAX_CHARS = 120_000
const DOC_CONTEXT_MAX_FILE_CHARS = 3_500
...
async function listBranchCommits(rootHandle: FileSystemDirectoryHandle): Promise<Map<string, string>> {
    const out = new Map<string, string>()
    try {
        const heads = await rootHandle.getDirectoryHandle(".git").then((git) => git.getDirectoryHandle("refs")).then((refs) => refs.getDirectoryHandle("heads"))
